...
public class BanderaFinDeJuego : MonoBehaviour
{
    ...
    public GameObject panelVictoria; // Arrastra aqu� la imagen de "Has Ganado"
    public GameObject hudJuego;      // Arrastra aqu� el Canvas de corazones para ocultarlo
    ...
    {
        // Si toca el jugador y no hemos terminado a�n
        if (collision.CompareTag("Player") && !juegoTerminado)
        {
            juegoTerminado = true;

            // 1. Animaci�n de la bandera
            if (anim != null) anim.SetTrigger("activar");

            // 2. Frenamos a la rana (F�sica y Script)
            Rigidbody2D playerRb = collision.GetComponent<Rigidbody2D>();
            if (playerRb != null)
            {
                playerRb.linearVelocity = Vector2.zero; // Frenamos en seco

                // Desactivamos el movimiento para que no salte m�s
